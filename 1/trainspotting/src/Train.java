...
public class Train extends Thread
{
  ...
  private final static Semaphore startUpperSema = new Semaphore(0);
  private final static Semaphore startLowerSema = new Semaphore(0);
  ...
  private void startEventHandler(SensorEvent sensor) throws CommandException, InterruptedException {
    if (this.direction == RIGHT) {

      // SENSOR 1
      if (sensorEventEquals(sensor, startEvent[0])) {
        if (semas[0].tryAcquire()) {
      ...
      // SENSOR 2
      else if (sensorEventEquals(sensor, startEvent[1])) {
        if (semas[0].tryAcquire()) {
          for (Semaphore s: semas) {
            System.out.println(s.availablePermits());
          }
      ...
      // SENSOR 3
      else if (sensorEventEquals(sensor, startEvent[2])) {
        for (Semaphore s: semas) {
          System.out.println(s.availablePermits());
        }
        semas[0].release();
      ...
      // SENSOR 4
      else if (sensorEventEquals(sensor, startEvent[3])) {
        for (Semaphore s: semas) {
          System.out.println(s.availablePermits());
        }
        semas[0].release();
